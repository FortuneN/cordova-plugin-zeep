	...
	public class Zeep : BaseCommand
	{
        public void zip(string options)
        {
            try
            {
                ...
                DispatchCommandResult(new PluginResult(PluginResult.Status.OK), CurrentCommandCallbackId);
            }
            catch (Exception e)
            {
                DispatchCommandResult(new PluginResult(PluginResult.Status.ERROR, e.Message), CurrentCommandCallbackId);
            }
        }
...
namespace System.IO
{
	public class ZipStorer
	{
		...
		#region CRC32 table
		static UInt32[] CrcTable = new UInt32[] {
